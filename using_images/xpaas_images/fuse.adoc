...
You can start using Fuse Integration Services by creating application and deploying it to OpenShift. You can use Application Development workflows to to create an application. These are: 
...
==== Simple steps to create an application from Maven Archetype Catalog
...
+
You can then use the Maven archetype catalog to create a sample project with the required resources. The command to create a sample project is:
+
----
...
----
[NOTE]
Replace [archetype-name] with the name of the archetype that you want to use. For example, karaf-camel-log-archetype will create a new Camel log example.
